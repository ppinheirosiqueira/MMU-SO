...
def LRU(mem, matriz_paginas, pages, size):
    pagemiss = 0
    while True:
        page = pages.pop(0)
        ...
        if x == None:
            ...
            matriz_paginas[menor_linha] = np.ones(size)
            matriz_paginas[:,menor_linha] = np.zeros(size)
        else:
            mem[x].update({"R": 1})
            text = f"Referenciou: {mem[x]}"
            matriz_paginas[mem[x]["line"]] = np.ones(size)
            matriz_paginas[:,mem[x]["line"]] = np.zeros(size)
...
mem = list()
size = int(100 * (random.choice([10,20,30,40,50])/100))
...
while True:
    ...
    if x == None:
        ...
        matriz_paginas[i-1] = np.ones(size)
        matriz_paginas[:,i-1] = np.zeros(size)
    else:
        mem[x].update({"R": 1})
    if i == size:
        break

LRU(mem, matriz_paginas, pages, size)